...
/**
 * Servlet implementation class AjaxMissionControl
 */
@WebServlet("/AjaxMissionControl")
@MultipartConfig
public class AjaxMissionControl extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AjaxMissionControl() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String reqtype = request.getParameter("modaltype");
		System.out.println(reqtype);
		System.out.println(request.getParameter("missiontitle"));
		System.out.println(request.getParameter("missionassgn"));
		
		if(reqtype.equals("mission")){
			
			processCreation(request, response, reqtype);
						
		} else if(reqtype.equals("subtask")){
	...
	protected void processCreation(HttpServletRequest request, HttpServletResponse response, String modaltype){
		...
		if(modaltype.equals("mission")){			
			AssignmentName = request.getParameter("missionassgn");
			description = request.getParameter("missiondesc");
			title = request.getParameter("missiontitle");
		} else {
			AssignmentName = request.getParameter("subassgn");
			description = request.getParameter("subdesc");
			if(request.getParameter("submission") != null || !request.getParameter("submission").equals("choice")){
				missionid = Integer.parseInt(request.getParameter("submission"));
			}  else {
		...
		try {
			...
			if(modaltype.equals("mission")){
				...
				filePart = request.getPart("file-1a");
				fileName = getFileName(filePart).replaceAll("\\s+", "_");
				
				filePartTest = request.getPart("file-1b");
				testFilename = getFileName(filePartTest).replaceAll("\\s+", "_");
				
				filePartQuestion = request.getPart("file-1c");
				questionFilename = getFileName(filePartQuestion).replaceAll("\\s+", "_");
			}
			...
			if(fileName != null && !fileName.trim().equals("") && testFilename != null && !testFilename.trim().equals("") && questionFilename != null && !questionFilename.trim().equals("")) {
				...
				if(contentlist.size()>0){
					...
					if(!assignment_state){
						...
						if(modaltype.equals("mission")){
							dbm.insertMissionDetails(title, description, AssignmentName);
						} else {
							dbm.insertSubtaskDetails(description, missionid, AssignmentName);
						}
					} else {
